...
export async function createMember(data: {
  ...
  if (userSession.session?.user.user_metadata.role !== "admin")
    return JSON.stringify({
      error: { message: "You don't have permission to create a member" },
    });
...
export async function updateMemberAdvanceById(
  ...
  user_id: string,
  data: {
    role: "admin" | "manager" | "new_user" | "verified_user";
    status: "active" | "resigned";
  }
) {
  const { data: userSession } = await readUserSession();
...
export async function deleteMemberById(user_id: string) {
  ...
  if (userSession.session?.user.user_metadata.role !== "admin")
    return JSON.stringify({
      error: { message: "You don't have permission to delete a member" },
    });
